...
def fetch_all_teachers():
    """
    Fetch all teacher information from the UIC staff API
    ...
    print("Fetching teacher data from UIC API...")
...
def convert_teachers_to_csv(teachers, output_dir="output"):
    ...
    for teacher in teachers:
        ...
        if name and name_en and name.strip() != name_en.strip():
            full_name = f"{name} {name_en}".strip()
        else:
            full_name = name or name_en
        name = full_name
        ...
        # Build URL
        url = f"https://staff.uic.edu.cn/{username}/en" if username else ""
