...
def read_excel_to_dataframes(excel_file_path):
    """
    Read an Excel file and convert all sheets to pandas Data
    ...
    try:
        ...
        try:
            excel_data = pd.read_excel(
                excel_file_str, sheet_name=None, engine=engine, header=1
            )
            ...
            if engine == "openpyxl":
                ...
                excel_data = pd.read_excel(
                    excel_file_str, sheet_name=None, engine="xlrd", header=1
                )
            else:
                print(f"xlrd failed, trying openpyxl: {first_error}")
                excel_data = pd.read_excel(
                    excel_file_str, sheet_name=None, engine="openpyxl", header=1
                )
...
def extract_year_semester_from_filename(filename):
    ...
    if year_match and semester_match:
        ...
        end_year_suffix = year_match.group(2)
        semester = semester_match.group(1)

        # Semester 2 (Spring) occurs in the second year of the academic year
        # e.g., AY2024-25 Semester 2 is Spring 2025, not 2024
        if semester == "2":
            # Construct full end year from suffix
            # e.g., if start_year is 2024 and suffix is '25', end_year is 2025
            end_year = int(str(start_year)[:2] + end_year_suffix)
            return str(end_year), semester
        else:
            # Semester 1 (Fall) occurs in the first year
            return str(start_year), semester

    return None, None


def extract_session_from_title(title):
    """
    Extract session number from course title

    Args:
        title (str): Course title with session (e.g., "Advanced Accounting I (1001)")

    Returns:
        str: Session number or empty string if not found
    """
    if pd.isna(title) or title == "":
        return ""

    # Pattern to match session in brackets at the end: "(1001)"
    session_match = re.search(r"\((\d+)\)\s*$", str(title).strip())
    if session_match:
...
def process_excel_to_offering_csv(excel_file_path, output_dir="output"):
    """
    Process an Excel file and create a CSV file with course offerings (v2)
    ...
    try:
        ...
        for sheet_name, df in dataframes.items():
            if (
                "Course List" in sheet_name
                or "Timetable" in sheet_name
                or "Export" in sheet_name
                or "Semester" in sheet_name
            ):
        ...
        required_columns = ["Course Code", "Teachers"]
        for col in required_columns:
            if col not in course_df.columns:
                print(f"Required column '{col}' not found in {excel_file_path}")
                print(f"Available columns: {list(course_df.columns)}")
                return None

        # Create offering DataFrame with required columns
        columns_to_extract = ["Course Code", "Teachers"]
        
        # Add session column if available
        if "Course Title & Session" in course_df.columns:
            columns_to_extract.append("Course Title & Session")
        
        # Add schedule column if available
        if "Class Schedule" in course_df.columns:
            columns_to_extract.append("Class Schedule")
        
        offering_df = course_df[columns_to_extract].copy()
        
        # Rename columns
        offering_df = offering_df.rename(
            columns={"Course Code": "course_code", "Teachers": "lecturer_name"}
        )

        # Extract session from "Course Title & Session"
        if "Course Title & Session" in offering_df.columns:
            offering_df["session"] = offering_df["Course Title & Session"].apply(
                extract_session_from_title
            )
            offering_df = offering_df.drop(columns=["Course Title & Session"])

        # Ensure schedule column exists
        if "Class Schedule" not in offering_df.columns:
            offering_df["Class Schedule"] = ""

        # Remove rows with missing course_code
        offering_df = offering_df.dropna(subset=["course_code"])

        # Handle missing lecturer names - fill with empty string
        offering_df["lecturer_name"] = offering_df["lecturer_name"].fillna("")
        ...
        offering_df["lecturer_name"] = offering_df["lecturer_name"].str.replace(
            r"^Course Convener:\s*", "", regex=True
        )
        offering_df["lecturer_name"] = offering_df["lecturer_name"].str.replace(
            r"^Instructor:\s*", "", regex=True
        )
        offering_df["lecturer_name"] = offering_df["lecturer_name"].str.replace(
            r"^Teacher:\s*", "", regex=True
        )
        offering_df["lecturer_name"] = offering_df["lecturer_name"].str.replace(
            r"^Lecturer:\s*", "", regex=True
        )

        # Remove any remaining empty strings after cleaning
        offering_df = offering_df[offering_df["lecturer_name"].str.strip() != ""]

        # Aggregate schedules by course_code, session, lecturer_name
        # Group and combine schedules with '&' for each unique session
        schedule_groups = offering_df.groupby(
            ["course_code", "session", "lecturer_name"], dropna=False
        )["Class Schedule"].apply(
            lambda x: " & ".join(filter(None, [str(s).strip() for s in x if pd.notna(s) and str(s).strip()]))
        ).reset_index()

        schedule_groups = schedule_groups.rename(columns={"Class Schedule": "schedule"})

        # Drop duplicate course_code, session, lecturer_name combinations to keep unique rows
        unique_offering = offering_df.drop_duplicates(
            subset=["course_code", "session", "lecturer_name"], keep="first"
        )

        # Merge with aggregated schedules
        offering_df = unique_offering[["course_code", "session", "lecturer_name"]].merge(
            schedule_groups, on=["course_code", "session", "lecturer_name"], how="left"
        )
        ...
        # Reorder columns to desired order: course_code, year, semester, session, lecturer_name, schedule
        offering_df = offering_df[
            ["course_code", "year", "semester", "session", "lecturer_name", "schedule"]
        ]

        # Fill empty session and schedule with empty strings
        offering_df["session"] = offering_df["session"].fillna("")
        offering_df["schedule"] = offering_df["schedule"].fillna("")
        ...
        offering_df = offering_df[
            ~offering_df["lecturer_name"].str.startswith("TBC", na=False)
        ]
        offering_df = offering_df[
            ~offering_df["lecturer_name"].str.startswith("TBA", na=False)
        ]
        ...
        # Create output filename (v2)
        output_filename = f"offering-v2-{year}-{semester_name}.csv"
...
def process_all_excel_files_departments():
    ...
    excel_files = [
        "input/Course List and Timetable (Semester 2 of AY2021-22)_20220213.xls",
        ...
        "input/Course List and Timetable_Semester 1 of AY2025-26_20250909.xlsx",
        "input/Course List and Timetable_Semester 2 of AY2025-26_20260109.xls",
    ]
...
def process_all_excel_files():
    """
    Process all Excel files and create CSV offerings (v2)
    """
    # Get the directory where this script is located
    script_dir = Path(__file__).parent
    ...
    if successful:
        print(f"\n✅ Successfully created CSV files (v2):")
        for file_path in successful:
            print(f"   - {file_path}")
    ...
    if dataframes:
        ...
        if main_sheet:
            ...
            if "Curriculum Type" in course_df.columns:
                print(
                    f"Curriculum types: {sorted(course_df['Curriculum Type'].unique())}"
                )
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--process":
        # Process all Excel files to CSV (v2)
        print("Processing all Excel files to CSV format (v2)...")
